      ...
      <div className="bg-surface-raised rounded-2xl border border-outline-soft/60 shadow-sm p-4 sm:p-6">
        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-outline-soft/60 border border-outline-soft/60 rounded-xl overflow-hidden">
            {rows.map((row) => (
              <div key={row.id} className="flex items-center gap-3 sm:gap-4 px-3 sm:px-4 py-3 hover:bg-surface-muted/50 transition-colors">
                <div className="p-2 sm:p-2.5 rounded-xl bg-brand-soft text-brand shrink-0">
                  <Clock size={16} />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-xs sm:text-sm text-ink">
                    <span className="font-semibold">{row.actor_name || 'Không xác định'}</span>{' '}
                    <span className="text-ink-muted">đã</span>{' '}
                    <span className="font-medium text-brand">{ACTION_LABELS[row.action] || row.action}</span>
                  </p>
                ...
                <span className="text-[11px] sm:text-xs text-ink-muted shrink-0 text-right">
