  ...
  if (!isManager) {
    return (
      <div className="bg-surface-raised rounded-2xl border border-outline-soft/60 shadow-sm p-6">
        <p className="text-sm text-ink-muted">Chỉ Quản lý mới có quyền cấu hình mục tiêu kỳ.</p>
      </div>
    );
  }

  return (
    <div className="bg-surface-raised rounded-2xl border border-outline-soft/60 shadow-sm p-4 sm:p-6 max-w-xl">
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-ink mb-1">Kỳ đánh giá</label>
          <select
            value={selectedPeriodId}
            onChange={(e) => handlePeriodChange(e.target.value)}
            className="w-full px-3 py-2.5 border border-outline-soft rounded-xl bg-surface-raised text-ink text-sm focus:outline-none focus:ring-2 focus:ring-brand/40"
          >
            {allPeriods.map((p) => (
        ...
        <div>
          <label className="block text-sm font-medium text-ink mb-1">Tỉ lệ mục tiêu (%)</label>
          <input
            type="number"
            min={0}
            max={100}
            ...
            placeholder="VD: 75"
          />
          <p className="text-xs text-ink-muted mt-1">Phần trăm nhân sự cần đạt mức xếp loại trở lên.</p>
        </div>

        <div>
          <label className="block text-sm font-medium text-ink mb-1">Mức xếp loại tối thiểu</label>
          <select
            value={grade}
