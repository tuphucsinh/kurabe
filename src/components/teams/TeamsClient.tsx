    ...
    <TeamsShell>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          {/* Cụm KPI Compact */}
          <div className="grid grid-cols-3 gap-1 bg-surface-raised rounded-xl border border-outline-soft/80 p-1 shadow-sm md:flex md:items-center md:px-1 md:py-0.5 md:gap-0 md:divide-x md:divide-outline-soft">
            <div className="px-1 py-2 md:px-4 flex flex-col md:flex-row items-center gap-0.5 md:gap-2" data-load-layer="light">
              ...
              <span className="text-[11px] md:text-sm text-ink-muted">tổng nhóm</span>
            </div>
            ...
            <div className="px-1 py-2 md:px-4 flex flex-col md:flex-row items-center gap-0.5 md:gap-2" data-load-layer="heavy">
              ...
              <span className="text-[11px] md:text-sm text-ink-muted">tiến độ</span>
            </div>
          </div>
          ...
          {isManager && (
            <button
              ...
              className="max-md:hidden px-6 py-3 bg-brand text-white rounded-xl font-bold hover:bg-brand-mid transition-all shadow-lg shadow-brand/20 flex items-center justify-center gap-2 group active:scale-95 shrink-0"
            >
      ...
      {isLightLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-3 md:gap-4" data-load-layer="light">
          {teamsData.map((team) => (
            <Link
              ...
              prefetch={false}
              href={`/teams/${team.id}`}
              className="group bg-surface-raised rounded-2xl border border-outline-soft shadow-sm hover:shadow-lg hover:border-brand/30 transition-all duration-300 flex flex-col overflow-hidden cursor-pointer"
            >
              ...
              <div className="p-4 pb-3 md:p-4 md:pb-3">
                ...
                <div className="flex items-center gap-3 md:hidden">
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="text-base font-black text-ink truncate leading-snug">{team.name}</h3>
                    <p className="text-xs text-ink-muted flex items-center gap-1.5 mt-0.5 truncate">
                      <UserIcon size={12} className="shrink-0" />
                      <span className="truncate">Leader: <span className="font-bold text-ink">{team.leaderName}</span></span>
                    </p>
                  </div>
                </div>
                ...
                <div className="max-md:hidden">
                  <div className="flex justify-between items-start mb-3">
                    <div className="p-2 rounded-xl bg-brand-soft text-brand group-hover:bg-brand group-hover:text-white transition-colors duration-300">
                      <Users size={20} />
                    </div>
                      ...
                      {isManager && (
                        <>
                          <button
                            ...
                            className="p-2.5 min-w-11 min-h-11 flex items-center justify-center rounded-lg text-ink-muted hover:text-brand hover:bg-brand-soft transition-all"
                          ...
                          <button
                            ...
                            className="p-2.5 min-w-11 min-h-11 flex items-center justify-center rounded-lg text-ink-muted hover:text-rose-600 hover:bg-rose-50 transition-all"
                            title="Xóa nhóm"
                          >
                            <Trash2 size={16} />
                          </button>
                  ...
                  <h3 className="text-lg font-black text-ink mb-1">{team.name}</h3>
                  <p className="text-sm text-ink-muted flex items-center gap-1.5">
                    <UserIcon size={14} />
                    Leader: <span className="font-bold text-ink">{team.leaderName}</span>
                  </p>
                </div>
              </div>
        ...
        <div className="bg-surface-raised rounded-3xl border border-dashed border-outline-soft p-12 min-h-[400px] flex items-center justify-center" data-load-layer="light">
          <EmptyState
            icon={Users}
