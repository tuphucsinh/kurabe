      ...
      {isLoading ? (
        <div className="space-y-6 md:space-y-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
            <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-100 h-64 animate-pulse bg-slate-50/80" />
            <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-100 h-64 animate-pulse bg-slate-50/80" />
          </div>

          {/* Skeleton Grid: Radar | Recent */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
            <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-100 h-72 animate-pulse bg-slate-50/80" />
            <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-100 h-72 animate-pulse bg-slate-50/80" />
          </div>
        </div>
      ) : error ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
            ...
            <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col">
              <h3 className="text-base sm:text-lg font-semibold text-slate-800 mb-4 sm:mb-6">Hoạt động gần đây</h3>
              <div className="space-y-3 sm:space-y-4 flex-1">
                {data.recentActivities.map((activity) => (
                  <div
                    key={activity.id}
                    className="flex items-start gap-3 sm:gap-4 p-2.5 sm:p-3 rounded-xl hover:bg-slate-50 transition-colors"
                  >
                    <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold flex-shrink-0 text-sm sm:text-base">
