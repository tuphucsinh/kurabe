...
export async function suggestCommentAction(input: {
  ...
  const prevText = input.previousComments.length
    ...
    : 'không có';
  const roundScoresText = input.roundScores.length
    ? input.roundScores.map((s, i) => `Vòng ${i + 1}: ${s} điểm`).join(', ')
    : '';
...
- ĐIỂM CÁC VÒNG ĐÃ CHẤM: ${roundScoresText || 'chưa có'}
...
2. Nếu chênh lệch điểm giữa các vòng ĐÁNG KỂ (≥10 điểm): thêm 1 câu nhận xét tiến bộ/giảm so với vòng trước. Nếu không chênh lệch: bỏ qua.
3. KỶ LUẬT (mã A*): không vi phạm → CHỈ 1 câu NGẮN nhưng DIỄN ĐẠT ĐA DẠNG theo từng người (vd: "Anh/chị duy trì kỷ luật và tác phong lao động tốt trong kỳ", "Không phát sinh vi phạm nội quy, chấm công ổn định", "Tinh thần chấp hành nội quy tốt, không có vấn đề kỷ luật"...). TUYỆT ĐỐI không viết y hệt câu giống nhau cho mọi người, không liệt kê tiêu chí 0 điểm, không nêu "theo dõi chấm công" khi không có vi phạm.
4. Có THAM KHẢO nhận xét vòng trước để bổ sung/nhất quán (nếu có).
5. KHÔNG nêu tổng điểm số. Kết 1 câu khuyến khích ngắn.
6. VIẾT GIỐNG NGƯỜI THẬT, BẮT CHƯỚC PHONG CÁCH MẪU (tự nhiên, thực tế) — TRÁNH giọng văn AI (không dùng "cho thấy sự nỗ lực", "đáng ghi nhận", "góp phần không nhỏ", "thể hiện rõ", liệt kê đều đều, cảm thán sáo rỗng). Mỗi nhân viên một cách viết khác nhau.
...
- Tổng điểm: ${input.totalScore}, xếp loại: ${input.grade}
- ĐIỂM CÁC VÒNG: ${roundScoresText || 'chưa có'}
...
4. Nếu chênh lệch điểm giữa các vòng ĐÁNG KỂ (≥10): 1 câu nhận xét tiến bộ/giảm.
5. Kỷ luật (A*) không vi phạm → KHÔNG nêu hoặc tối đa nửa câu, DIỄN ĐẠT ĐA DẠNG theo từng người — không lặp lại cùng một câu.
6. Kết 1 câu khuyến khích.
