...
const GROUP_NAMES: Record<'leader' | 'staff' | 'worker', string> = {
  leader: 'Quản lý',
  staff: 'Nhân viên',
  worker: 'Công nhân',
};
  ...
  for (const group of ['leader', 'staff', 'worker'] as const) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (row.minScore === null && i !== rows.length - 1) {
        return `Xếp loại ${row.grade} (${groupName}): chỉ xếp loại thấp nhất (${VALID_GRADES[VALID_GRADES.length - 1]}) được bỏ trống điểm tối thiểu.`;
      }
      ...
      if (
        ...
        return `Xếp loại ${row.grade} và ${next.grade} (${groupName}): dải điểm chồng lấn.`;
      }
    }
  }
  return null;
