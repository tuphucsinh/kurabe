...
import { User, Team, Evaluation, CriteriaGroup, EvaluationRound } from '@/types';
...
export interface DashboardData extends DashboardPrimaryData, DashboardSecondaryData {}
...
export function computeDashboardPrimaryData(
...
export function computeDashboardSecondaryData(
  evaluations: Evaluation[],
  users: User[],
...
export interface DashboardSource {
  shared: Promise<DashboardSharedData>;
  primary: Promise<DashboardPrimaryData | null>;
  secondary: Promise<DashboardSecondaryData | null>;
}

export function createDashboardSource(
  periodId: string,
  viewer: User | null
): DashboardSource {
  if (!periodId) {
  ...
  // Primary derived promise: KPI / grade / team status (no criteria)
  const primary = shared
    .then(({ evaluations, users, teams }) => computeDashboardPrimaryData(evaluations, users, teams))
    .catch((error) => {
      console.error('Error in dashboardSource.primary:', error);
      return null;
    });

  // Secondary derived promise: recent / radar / anomaly / criteria (fetches criteria lazily)
  const secondary = Promise.all([shared, getAllCriteriaGroups()])
    .then(([{ evaluations, users }, criteriaGroups]) =>
      computeDashboardSecondaryData(evaluations, users, criteriaGroups)
    )
    .catch((error) => {
      console.error('Error in dashboardSource.secondary:', error);
      return null;
    });
