...
export default function SupportPage() {
  ...
  return (
    <div className="px-6 md:px-10 lg:px-12 py-8 lg:py-5 space-y-8 lg:space-y-4 animate-in fade-in duration-500 w-full max-w-[1600px] mx-auto print:p-0 print:space-y-6">
      <div className="rounded-[24px] border border-outline bg-surface-raised p-5 md:p-6 shadow-sm print:border-none print:shadow-none print:p-0">
        <div className="flex flex-col sm:flex-row justify-between items-start gap-6">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.22em] text-ink-muted">HƯỚNG DẪN</p>
            <h1 className="mt-2 text-2xl md:text-3xl font-black text-ink">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-2xl bg-brand-strong px-5 py-3 text-sm font-black text-white transition-colors hover:bg-brand shadow-sm whitespace-nowrap print:hidden active:scale-95 cursor-pointer"
          >
            <Printer size={18} />
            In Hướng Dẫn (A4)
          </button>
      ...
      <div className="grid gap-8 xl:grid-cols-[1.2fr_0.8fr]">
        <section className="space-y-6">
          <div id="tro-giup-ai" className="rounded-[24px] border-2 border-emerald-300 bg-gradient-to-br from-white via-emerald-50/50 to-emerald-100/70 p-6 md:p-8 shadow-lg relative overflow-hidden">
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              ...
              <div>
                ...
                <h2 className="text-2xl md:text-3xl font-black text-ink">Trợ lý AI — giúp bạn ngay khi gặp lỗi hoặc chưa biết cách làm</h2>
              </div>
            </div>
            <div className="mt-5 space-y-3 text-[15px] leading-7 text-ink">
          ...
          <div id="workflow" className="rounded-[24px] border border-outline bg-surface-raised p-6 shadow-sm">
            <div className="flex items-center gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-brand-soft text-brand">
                <ClipboardCheck size={22} />
              </div>
              <div>
                <h2 className="text-xl font-black text-ink">Quy trình 3 vòng đánh giá</h2>
            ...
            <div className="mt-6 space-y-4">
              {ALL_GUIDE_ROLES.map((role) => (
                <div key={role} className="rounded-[20px] border border-outline-soft bg-surface-muted p-4">
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    {roleWorkflows[role].map((step, index) => (
                      <div key={`${role}-wf-${index}`} className="flex items-center gap-2">
                        <div className="min-w-[138px] rounded-2xl border border-outline-soft bg-surface-raised px-3 py-3 shadow-sm">
                          <p className="text-[11px] font-black uppercase tracking-wider text-ink-muted">{step.round}</p>
                          <p className="mt-1 text-xs font-semibold leading-5 text-ink">{step.evaluator}</p>
                        </div>
                        {index < roleWorkflows[role].length - 1 && (
                          <div className="flex h-7 w-7 items-center justify-center rounded-full bg-surface-muted text-ink-muted">
                            <ArrowRight size={14} />
                          </div>
                        )}
                      </div>
          ...
          <div id="huong-dan-vai-tro" className="rounded-[24px] border-2 border-cyan-200 bg-gradient-to-br from-white via-cyan-50/40 to-cyan-100/60 p-6 md:p-8 shadow-md">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-2xl md:text-3xl font-black text-ink">Hướng dẫn theo vai trò của bạn</h2>
              ...
              {isManager ? (
                <div className="flex flex-wrap items-center gap-2 p-1.5 bg-surface-raised rounded-2xl border-2 border-cyan-200 shadow-sm">
                  {ALL_GUIDE_ROLES.map((role) => (
                    <button
                      key={role}
                      type="button"
                      ...
                      className={`px-5 py-2.5 rounded-xl text-sm md:text-base font-black transition-all cursor-pointer ${
                        viewRole === role
                          ? 'bg-brand-strong text-white shadow-md scale-[1.02]'
                          : 'text-ink-muted hover:text-brand-strong hover:bg-cyan-50'
                      }`}
                    >
                      {roleLabels[role]}
                    </button>
                ...
                <div className="inline-flex items-center gap-2 rounded-2xl bg-surface-raised border-2 border-cyan-200 px-4 py-2 text-sm font-black text-ink shadow-sm">
            ...
            <div className="mt-6 grid gap-4">
              {activeGuide.steps.map((step, index) => (
                <div
                  ...
                  className="grid gap-4 rounded-[20px] border border-outline-soft bg-surface-muted p-4 md:grid-cols-[56px_minmax(0,1fr)] md:items-start"
                >
                  <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-surface-raised text-lg font-black text-cyan-800 shadow-sm shrink-0">
                    {String(index + 1).padStart(2, '0')}
                  </div>
                  <div className="space-y-3">
                    <div>
                      <h3 className="text-sm font-black text-ink md:text-base">{step.title}</h3>
                      <p className="mt-2 text-sm leading-7 text-ink-muted whitespace-pre-line">{step.body}</p>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <section className="space-y-6">
          <div className="rounded-[24px] border border-outline bg-brand-strong p-6 text-white shadow-sm">
            <div className="flex items-center gap-3">
          ...
          <div className="rounded-[24px] border border-outline bg-surface-raised p-6 shadow-sm">
            <div className="flex items-center gap-3">
            ...
            {activeGuide.faq && activeGuide.faq.length > 0 ? (
              <div className="mt-6 space-y-3">
                {activeGuide.faq.map((item, idx) => (
                  <div key={idx} className="rounded-[18px] border border-outline-soft bg-surface-muted p-4">
              ...
              <p className="mt-6 text-sm text-ink-muted">Chưa có câu hỏi cho vai trò này.</p>
            )}
          </div>
        </section>
      </div>
