...
import { CompareStaticFrame } from './loading';
            ...
            {employeeName ? (
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <span className="text-xs text-ink-muted font-medium">
                  {employeeCode || 'No Code'}
                </span>
              </div>
            ) : null}
          </div>
        </div>
        {activeVisibleRound !== undefined && activeVisibleRound !== null && (
...
export default function ComparePageClient({ employeeId, scope }: ComparePageClientProps) {
  ...
  if (loadingUser || loadingEval || loadingCriteria || !pageData) {
  ...
  if (accessState.mode === 'blocked') {
    return (
      <CompareFrame onBack={handleBack}>
        <div className="min-h-[50vh] flex flex-col items-center justify-center p-8 text-center space-y-4" role="alert">
