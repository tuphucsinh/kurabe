        ...
        <div className="flex flex-col gap-4 sm:gap-6">
            ...
            <div className="flex items-center gap-2 sm:gap-3 overflow-x-auto pb-3 scrollbar-hide touch-pan-x">
              {[1, 2].map((i) => (
                <div key={i} className="flex items-center gap-3 sm:gap-4 shrink-0">
                  ...
                  <div className="min-w-[144px] sm:min-w-[160px] p-3 sm:p-4 rounded-2xl border border-outline-soft bg-surface-raised shadow-sm flex flex-col items-center text-center space-y-2">
          ...
          <section data-load-phase="primary" data-load-layer="changed-criteria">
            ...
            <div className="md:hidden space-y-3">
              {[1, 2, 3].map((i) => (
                <div key={i} className="p-3 sm:p-3.5 rounded-xl border border-outline-soft bg-surface-raised shadow-sm space-y-2">
                  ...
                  <div className="grid grid-cols-2 min-[480px]:grid-cols-3 sm:grid-cols-4 gap-2 pt-2 border-t border-outline-soft/40">
                    {[1, 2].map((r) => (
                      <div key={r} className="p-2 rounded-lg border border-outline-soft/50 bg-surface/50 flex flex-col items-center justify-center text-center space-y-1.5 min-h-[52px]">
                ...
                <table className="w-full text-left border-collapse min-w-[640px]">
                  <thead>
                    <tr className="bg-surface/50 border-b border-outline-soft">
                      ...
                      <th className="px-3 py-3 text-[11px] font-black text-ink-muted uppercase tracking-wider text-center min-w-[84px]">
          ...
          <section data-load-phase="secondary" data-load-layer="comments">
            ...
            <div className="grid gap-3 sm:gap-4 grid-cols-1 md:grid-cols-2">
              {[1, 2].map((i) => (
                <div key={i} className="p-4 rounded-2xl border border-outline-soft bg-surface-raised shadow-sm flex flex-col gap-3">
          ...
          <section className="mt-0" data-load-phase="secondary" data-load-layer="unchanged">
            <div className="bg-surface-raised rounded-2xl border border-outline-soft overflow-hidden">
              <div className="px-4 sm:px-6 py-3 bg-surface/30 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <Skeleton className="h-3 w-36 rounded" />
                <Skeleton className="h-3 w-28 rounded" />
              </div>
              <div className="p-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  <div key={i} className="flex items-center justify-between p-2.5 rounded-xl bg-surface/20 border border-outline-soft/50">
