...
## Đã fix — 2026-08-17 (đợt quét toàn dự án sau review prod-fix)
- [x] **deleteEvaluationPeriod hard-delete không check (mục HANDOFF #4)**: giờ chặn xóa kỳ đang Active ("hãy Đóng kỳ trước") + dọn ai_summaries theo period_id trước khi xóa period (actions/period.ts).
- [x] **Lint 21 warning → 0**: dọn unused imports/vars 11 file (Sidebar, BatchResultMessageModal, TeamsClient, EmployeesClient, teams/[id], settings, dashboard, criteria/loading, AnomalyAlertCard, export.ts bỏ param `_viewer` thừa + caller); support/page img icon inline thêm eslint-disable có lý do.
- [x] **`.eslintignore` deprecated (ESLint 9)**: xóa file, gộp `dist/**` vào `globalIgnores` của eslint.config.mjs (flat config đã phủ hết các mục còn lại).
