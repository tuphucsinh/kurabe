...
- Previously recorded production deployment: source SHA `76221ca0a3e2813a2743a0ad74d3be7fb57306d4`, alias `https://lykiv.vercel.app`, closure evidence `/home/pi5/hermes-artifacts/kurabe-execution/production-release-closure-76221ca.log`. Deployment of current canonical `f8b6067` is the explicit next operation in this normalization, not a new product phase.
...
3. Push `main`, read back the remote SHA, then deploy the exact pushed canonical application through the existing Vercel procedure without migration/data writes.
4. Read back deployment, health, service, migration/write, and residue evidence.

After deployment, this file receives only a docs-only closure update recording the application SHA versus any later documentation SHA. F01–F08 remain untouched.
