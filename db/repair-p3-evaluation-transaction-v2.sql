...
DECLARE
  ...
  v_comment text;
BEGIN
  SELECT to_regprocedure('public.save_evaluation_round_transaction(uuid, integer, uuid, jsonb, jsonb, text, numeric, text, boolean, timestamptz, integer, uuid, text, text, boolean)') INTO v_proc_oid;
  IF v_proc_oid IS NULL THEN
    RAISE EXCEPTION 'REPAIR_BLOCKED: expected P3 function is absent';
  END IF;
  SELECT obj_description(v_proc_oid, 'pg_proc') INTO v_comment;
  IF v_comment IS DISTINCT FROM 'kurabe:p3:candidate:v1:function:save_evaluation_round_transaction' THEN
    RAISE EXCEPTION 'REPAIR_BLOCKED: function provenance marker mismatch';
  END IF;
END $$;
