...
-- KURABE QAQC — Rollback P98M3T01: Restore Prior Evaluator Authorization RPC Definition
-- DO NOT EXECUTE DIRECTLY WITHOUT EXPLICIT APPROVAL AND GUC CONFIRMATION
--
-- CRITICAL OPERATIONAL CONSTRAINTS:
-- 1. Candidate Only: Direct pg_catalog/information_schema is unknown/blocked.
...
--    The function is inspected in pg_description prior to rolling back.
--    If the function exists with an absent or mismatched provenance marker,
--    the script fails closed with RAISE EXCEPTION to prevent modifying
--    unowned database objects.
-- 4. Restores Prior Function Definition:
--    Restores the prior reviewed function definition with canonical check:
--      `IF v_round.evaluator_id != p_actor_id THEN`
--    and provenance marker 'kurabe:p3:candidate:v1:function:save_evaluation_round_transaction'.
-- 5. Exact Signature Only:
--    Restores only the exact function signature:
...
-- 6. Zero Business Data Mutation:
--    This script ONLY restores the RPC function definition and permissions.
--    It DOES NOT modify, delete, or truncate business data rows in any table.
-- ============================================================
...
BEGIN
  ...
  IF v_proc_oid IS NULL THEN
    RAISE EXCEPTION 'ROLLBACK_BLOCKED: Function public.save_evaluation_round_transaction does not exist.';
  END IF;
  ...
  IF v_comment IS DISTINCT FROM 'kurabe:p98:candidate:v1:function:save_evaluation_round_transaction' THEN
    RAISE EXCEPTION 'PROVENANCE_MISMATCH: Function public.save_evaluation_round_transaction exists but comment "%" does not match expected marker "kurabe:p98:candidate:v1:function:save_evaluation_round_transaction". Aborting.', v_comment;
  END IF;
END $$;

-- ------------------------------------------------------------
-- 3. RESTORE PRIOR REVIEWED FUNCTION DEFINITION (Canonical v_round.evaluator_id != p_actor_id)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 4. RESTORE PRIOR PROVENANCE MARKER & SERVICE-ROLE PERMISSIONS
-- ------------------------------------------------------------
