...
Current context: Product decision is `CURRENT_AUTH_MODE=OPTIONAL_PASSWORD`, `STRICT_PASSWORD_GO_LIVE=DEFERRED`, and `KURABE_REQUIRE_PASSWORD_LOGIN` must be effectively false. Prior source candidate review is not live schema evidence; the last P98M2T05 preflight is blocked by the pre-existing `public.login_attempts` collision. P98M2T09 and P98M2T10 must be PASS and canonical before a fresh production preflight; no prior catalog/baseline may be reused.
